...
public class Simulator2 {
	...
	private static Player loadPlayer() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
        ...
        ClassLoader loader = Simulator2.class.getClassLoader();
        if(loader == null)
...
//		p = 3 --> 1.5x means Math.ceil(21 * p * 1.5) = 95, 1x + extra for 1 family member = 21 * (p + 1) = 84 
		
		
		String[] argsChoices = {"g1", "g2", "g3", "g4", "g5"};
		int maxWeeks = 104;
		int[] seedsChoices = {8923, 537691, 2948376};
		
		String[] maps = {
				"test_50x50Spiral.map",
				"test_50x50Bubbles.map",
			    "test_25x25Diagonal.map",
			    "test_aa20.map",
			    "test_50x50Easter.map",
			    "test_nicu2.map",
			    "test_labyrinth.map",
			    "test_puzzle1.map",
			    "test_group3Final.map",
			    "test_skeleton2.map",
			    "test_group5Final.map",
			    "test_50x50Complex.map",
			    "test_100x100Complex.map"
		};

		for(int i = 0; i < seedsChoices.length; i++) {
			int seed = seedsChoices[i];
			System.out.println("Current seed: " + seed);
			for(int j = 11; j < maps.length; j++) {
				String map = maps[j];
				System.out.println("\tCurrent map (primary): " + map);
				for(int k = 0; k < argsChoices.length; k++) {
					System.out.println("\t\tCurrent team (primary): " + argsChoices[k]);

					File resultsFolder = new File("tournament_results2/seed_" + seed + "/" + map.substring(0, map.indexOf(".map")));
					resultsFolder.mkdirs();
					resultsDirectoryPath = resultsFolder.getAbsolutePath() + "/" + argsChoices[k] + "_simulations.csv";
					FileWriter csvWriter = new FileWriter(resultsDirectoryPath, true);
					csvWriter.append("Budget,Simulation Time,Time Remaining,Chemicals Used,Red Chemicals Used,Green Chemicals Used,Blue Chemicals Used,Chemicals Remaining\n");
					csvWriter.flush();
					csvWriter.close();
					for(int l = 0; l < primaryBudgets.length; l++) {
						resultsBudget = primaryBudgets[l];
						System.out.println("\t\t\tCurrent budget (primary): " + resultsBudget);
						String argsString = "--team " + argsChoices[k] + " -m " + map + " -b " + resultsBudget + " -r " + turns + " -s " + seed;
						setup();
						parseCommandLineArguments(argsString.split(" "));
						readMap();
						runSimulation();
					}
				}
