...
public class Player extends menu.sim.Player {
	...
	List<Integer> calcFreqMeals(Pantry pantry, int size, List<FamilyMember> familyMembers) {
		...
		int extraBreakfastAllocation = (int) ((pantrySizeNew - breakfast) * (meanBreakfastSatisfaction
				/ (meanBreakfastSatisfaction + meanLunchSatisfaction + meanDinnerSatisfaction)));
		int extraLunchAllocation = (int) ((pantrySizeNew - lunch) * (meanLunchSatisfaction
				/ (meanBreakfastSatisfaction + meanLunchSatisfaction + meanDinnerSatisfaction)));
		int extraDinnerAllocation = (int) ((pantrySizeNew - dinner) * (meanDinnerSatisfaction
				/ (meanBreakfastSatisfaction + meanLunchSatisfaction + meanDinnerSatisfaction)));
	...
	List<FoodType> calcOrderRanksLunchForEachHelper(FamilyMember familyMembers) {
		Food f = new Food();
		List<FoodType> allBreakfasts = f.getFoodTypes(MealType.LUNCH);
		Map<FoodType, Double> lowestPerson = new HashMap<>();
		for (FoodType food : allBreakfasts) {
			double lowest = 1.0;
			lowest = familyMembers.getFoodPreference(food);
			lowestPerson.put(food, lowest);
		}
		return sortByValue(lowestPerson);
	}
	...
	// 2.) rank dinner items based on the mean satisfaction of all the members
	// (bring up the floor of everyone's satisfaction)
	...
	// based on cutoffs, what is already in the pantry, rankings, put higher
	// priority in the shopping list on the people that are least satisfied
	List<FoodType> calcBreakfast(Pantry pantry, MealHistory mealHistory) {
		...
		for (FamilyMember member : this.members) {
			for (int i = 0; i < 3; i++) {
				FoodType topFood = breakfastRanks.get(member).get(i);
		...
		/*
		 * for(int i = 4; i < breakfastRanks.size(); i++) { FoodType badFood =
		 * breakfastRanks.get(i); int freqBad =
		 * map.get(MealType.BREAKFAST).get(badFood); breakfasts = addFoods(breakfasts,
		 * badFood, (int) (difference/5*1.5)); }
		 */
	...
	List<FoodType> calcLunch(Pantry pantry, MealHistory mealHistory) {
		...
		for (FamilyMember member : this.members) {
			for (int i = 0; i < 5; i++) {
				FoodType topFood = lunchRanks.get(member).get(i);
		...
		/*
		 * for(int i = 6; i < lunchRanks.size(); i++) { FoodType badFood =
		 * lunchRanks.get(i); int freqBad = map.get(MealType.LUNCH).get(badFood);
		 * 
	...
	// based on cutoffs, what is already in the pantry, rankings, put higher
	// priority in the shopping list on the food that brings up the floor
	List<FoodType> calcDinner(Pantry pantry, MealHistory mealHistory, List<FamilyMember> familyMembers) {
		...
		// System.out.println("existing meals " +
		// pantry.getNumAvailableMeals(MealType.DINNER));
		...
		/*
		 * for(int i = 6; i < dinnerRanks.size(); i++) { FoodType badFood =
		 * dinnerRanks.get(i); int freqBad = map.get(MealType.DINNER).get(badFood);
		 * 
	...
	ShoppingList combineShoppingLists(List<FoodType> breakfasts, List<FoodType> lunches, List<FoodType> dinners,
			Pantry pantry) {
		ShoppingList shoppingList = new ShoppingList();
	...
	public Planner planMeals(Integer week, List<FamilyMember> familyMembers, Pantry pantry, MealHistory mealHistory) {
		...
		for (Day day : Day.values()) {
			...
			List<FoodType> maxAvailableBreakfastMeal = new ArrayList<>();
			for (FamilyMember member : familyMembers) {
				// System.out.println(getRandomAvailableMeal(pantry, MealType.BREAKFAST,
				// member).toString());
				maxAvailableBreakfastMeal.add(getRandomAvailableMeal(pantry, MealType.BREAKFAST, member));
			}
			// System.out.println("HERE4");
			int satisfied = 0;
			int left = familyMembers.size();
			// System.out.println(maxAvailableBreakfastMeal.toString());
			for (int i = 0; i < this.members.size(); i++) {
				planner.addMeal(day, familyMembers.get(i).getName(), MealType.BREAKFAST,
						maxAvailableBreakfastMeal.get(i));
				pantry.removeMealFromInventory(maxAvailableBreakfastMeal.get(i));
			}
			// System.out.println("HERE^");
			/*
			 * for (FoodType food : maxAvailableBreakfastMeal) { if
			 * (pantry.getNumAvailableMeals(food) < left) { left -=
			 * pantry.getNumAvailableMeals(food); int i = pantry.getNumAvailableMeals(food);
			 * System.out.println(i + " " + pantry.getNumAvailableMeals(food)+satisfied);
			 * for (int j = satisfied; j < pantry.getNumAvailableMeals(food)+satisfied && j
			 * < this.members.size(); j++) { System.out.println("HERE5");
			 * planner.addMeal(day, familyMembers.get(j).getName(), MealType.BREAKFAST,
			 * food); pantry.removeMealFromInventory(food); } satisfied += i; } else { for
			 * (int i = satisfied; i < familyMembers.size(); i++) { planner.addMeal(day,
			 * familyMembers.get(i).getName(), MealType.BREAKFAST, food);
			 * pantry.removeMealFromInventory(food); } break; } }
			 */

			List<FoodType> maxAvailableLunchMeal = new ArrayList<>();
			for (FamilyMember member : familyMembers) {
				maxAvailableLunchMeal.add(getRandomAvailableMeal(pantry, MealType.LUNCH, member));
			}
			satisfied = 0;
			left = familyMembers.size();
			// System.out.println(maxAvailableLunchMeal.toString());
			for (int i = 0; i < this.members.size(); i++) {
				planner.addMeal(day, familyMembers.get(i).getName(), MealType.LUNCH, maxAvailableLunchMeal.get(i));
				pantry.removeMealFromInventory(maxAvailableLunchMeal.get(i));
			}
			/*
			 * for (FoodType food : maxAvailableLunchMeal) { if
			 * (pantry.getNumAvailableMeals(food) < left &&
			 * pantry.getNumAvailableMeals(food) > 0) { left -=
			 * pantry.getNumAvailableMeals(food); int i = pantry.getNumAvailableMeals(food);
			 * // System.out.println(i); for (int j = satisfied; j <
			 * pantry.getNumAvailableMeals(food) + satisfied; j++) { planner.addMeal(day,
			 * familyMembers.get(j).getName(), MealType.LUNCH, food);
			 * pantry.removeMealFromInventory(food); } satisfied += i; } else if
			 * (pantry.getNumAvailableMeals(food) > 0) { for (int i = satisfied; i <
			 * familyMembers.size(); i++) { planner.addMeal(day,
			 * familyMembers.get(i).getName(), MealType.LUNCH, food);
			 * pantry.removeMealFromInventory(food); } break; } }
			 */
		...
		for (Day day : Day.values()) {
			FoodType maxAvailableDinnerMeal = getRandomAvailableMeal(pantry, MealType.DINNER, familyMembers.get(0)); // this.dinnerRanks.get(getRandomAvailableMeal());
			...
			for (int i = 0; i < numDinners; i++) {
				MemberName memberName = this.members.get(i).getName();
	...
	private FoodType getRandomAvailableMeal(Pantry pantry, MealType mealType, FamilyMember member) {
		/*
		 * FoodType maximumAvailableMeal = null; int maxAvailableMeals = -1;
		 * for(FoodType foodType : Food.getFoodTypes(mealType)) { int numAvailableMeals
		 * = pantry.getNumAvailableMeals(foodType); if(numAvailableMeals >
		 * maxAvailableMeals) { maxAvailableMeals = numAvailableMeals;
		 * maximumAvailableMeal = foodType; } } return maximumAvailableMeal;
		 */

		Random rand = new Random();
		int randomNum = rand.nextInt(this.members.size());
		int randomNum2 = rand.nextInt(3);
		FamilyMember val = null;
		for (int i = 0; i < this.members.size(); i++) {
			val = member.getName().equals(this.members.get(i).getName()) ? this.members.get(i) : val;
		}
		if (mealType == MealType.LUNCH) {
			randomNum2 = rand.nextInt(this.lunchRanks.get(val).size());
			//
			if (pantry.containsMeal(lunchRanks.get(val).get(randomNum2))) {
				return lunchRanks.get(val).get(randomNum2);
			} else {
				// System.out.println("HERE6");
				return pantry.getAvailableFoodTypes(MealType.LUNCH).get(0);
			}
		} else if (mealType == MealType.BREAKFAST) {
			randomNum2 = rand.nextInt(this.breakfastRanks.get(val).size());
			//
			if (pantry.containsMeal(breakfastRanks.get(val).get(randomNum2))) {
				return breakfastRanks.get(val).get(randomNum2);
			} else {
				// System.out.println("HERE5");
				return pantry.getAvailableFoodTypes(MealType.BREAKFAST).get(0);
			}
		} else {
			FoodType f = dinnerRanks.get(randomNum2);
			if (pantry.containsMeal(f)) {
				return f;
			} else {
				// System.out.println("HERE5");
				return pantry.getAvailableFoodTypes(MealType.DINNER).get(0);
			}
		}
	}

	/*
	 * private List<FoodType> getMaximumAvailableMealLunch(Pantry pantry, MealType
	 * mealType, List<FamilyMember> members) { int i = 0; int quantity =
	 * members.size(); ArrayList<FoodType> foo = new ArrayList<FoodType>(); while (i
	 * < this.lunchRanks.size()) { if
	 * (pantry.getNumAvailableMeals(this.lunchRanks.get(i)) >= quantity) {
	 * foo.add(this.lunchRanks.get(i)); return foo; } else {
	 * foo.add(this.lunchRanks.get(i)); quantity -=
	 * pantry.getNumAvailableMeals(this.lunchRanks.get(i)); } i++; } return foo; /*
	 * FoodType maximumAvailableMeal = null; int maxAvailableMeals = 21; for
	 * (FoodType foodType : Food.getFoodTypes(mealType)) { int ranking =
	 * this.breakfastRanks.indexOf(foodType); if (maxAvailableMeals > ranking) {
	 * maxAvailableMeals = ranking; maximumAvailableMeal = foodType; } } return
	 * maximumAvailableMeal;
	 */
	// }
	...
	/*
	 * private List<FoodType> getMaximumAvailableMeal(Pantry pantry, MealType
	 * mealType, List<FamilyMember> members) { FoodType maximumAvailableMeal =
	 * FoodType.BREAKFAST1; int i = 0; int quantity = members.size();
	 * ArrayList<FoodType> foo = new ArrayList<FoodType>(); while (i <
	 * this.breakfastRanks.size()) { if
	 * (pantry.getNumAvailableMeals(this.breakfastRanks.get(i)) >= quantity) {
	 * foo.add(this.breakfastRanks.get(i)); return foo; } else {
	 * foo.add(this.breakfastRanks.get(i)); quantity -=
	 * pantry.getNumAvailableMeals(this.breakfastRanks.get(i)); } i++; } return foo;
	 * /* FoodType maximumAvailableMeal = null; int maxAvailableMeals = 21; for
	 * (FoodType foodType : Food.getFoodTypes(mealType)) { int ranking =
	 * this.breakfastRanks.indexOf(foodType); if (maxAvailableMeals > ranking) {
	 * maxAvailableMeals = ranking; maximumAvailableMeal = foodType; } } return
